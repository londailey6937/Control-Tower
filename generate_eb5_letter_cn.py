...
def build():
    ...
    pdf.table(
        ["层级", "服务内容", "月费"],
        [
            ["入门版", "Control Tower SaaS许可证，只读仪表板", "$500-$2,000"],
            ["专业版", "全程项目管理，审批管理和提交", "$10,000-$25,000"],
            ["企业版", "端到端：监管+项目管理+供应商+美国设立", "$50,000+/项目"],
        ],
        [30, 85, 45]
    )

    pdf.txt(
        "大多数客户从专业版开始。一个专业版客户每月$15,000，年收入$180,000。"
        "只需3-5个客户，公司年收入就能达到$500K-$900K。"
    )
    ...
    pdf.sub("示例：第二年，5个活跃客户")
    pdf.table(
        ["项目", "年度金额"],
        [
            ["收入（5个客户 x 月均$15K）", "$900,000"],
            ["薪酬福利（50%）", "$450,000"],
            ["运营（15%）", "$135,000"],
            ["业务拓展（10%）", "$90,000"],
            ["技术（5%）", "$45,000"],
    ...
    pdf.txt(
        "5个客户时，公司可以支撑整个团队有竞争力的薪酬，并向股东分配$180,000。"
        "随着第3-5年客户增加，薪资和分红都会增长。"
    )
