...
export async function fetchMessages(): Promise<DbMessage[]> {
  ...
  if (error) { console.error("Supabase fetchMessages:", error); return []; }
  return data ?? [];
}

export async function fetchMessagesByQuestion(qNum: number): Promise<DbMessage[]> {
  const { data, error } = await supabase
    .from("messages")
    .select("*")
    .eq("q_num", qNum)
    .order("created_at", { ascending: true });
  if (error) { console.error("Supabase fetchByQ:", error); return []; }
  return data ?? [];
}

export async function insertMessage(msg: Omit<DbMessage, "id" | "created_at">): Promise<DbMessage | null> {
  const { data, error } = await supabase
    .from("messages")
    .insert(msg)
    .select()
    .single();
  if (error) { console.error("Supabase insertMessage:", error); return null; }
  return data;
}

export async function markMessageRead(msgId: string, role: string): Promise<void> {
  // Use Postgres array_append to atomically add the role
  const { error } = await supabase.rpc("mark_read", { msg_id: msgId, reader: role });
...
export async function deleteMessages(qNum: number): Promise<boolean> {
  const { error } = await supabase
    ...
    .delete()
    .eq("q_num", qNum);
  if (error) { console.error("Supabase deleteMessages:", error); return false; }
  return true;
}
...
export function subscribeMessages(onInsert: MessageCallback, onUpdate?: MessageCallback): void {
  ...
  _channel = supabase
    ...
    .on(
      ...
      (payload) => { if (onUpdate) onUpdate(payload.new as DbMessage); },
    )
    .subscribe();
}
