...
def build():
    ...
    # ── WHY GRESHAM ─────────────────────────────────────
    pdf.sec("为什么选择Gresham, Oregon")

    pdf.txt(
        "Gresham就在波特兰东边——离机场只有20分钟车程。它符合目标就业区（TEA）条件，"
        "这意味着EB-5投资门槛是$800,000而不是$1,050,000。"
    )

    pdf.txt(
        "我建议你卖掉现在的房子，在Gresham买新房。那里有很漂亮的房子——我看到一套"
    ...
    pdf.txt(
        "510kBridge将注册在俄勒冈州Gresham——一个符合条件的目标就业区（TEA），"
