...
const URL = 'https://www.fda.gov/medical-devices/device-advice-comprehensive-regulatory-assistance/guidance-documents-medical-devices-and-radiation-emitting-products';
...
(async () => {
  ...
  await page.waitForSelector('table.lcds-datatable--cdrh tbody tr', { timeout: 30000 });
  ...
  if (hasAllOption) {
    ...
    await page.waitForFunction(() => {
      ...
      const rows = document.querySelectorAll('table.lcds-datatable--cdrh tbody tr');
  ...
  const docs = await page.evaluate(() => {
    ...
    for (const row of rows) {
      ...
      const title = summaryLink ? summaryLink.textContent.trim() : cells[0].textContent.trim();
      ...
      results.push({ title, pageUrl, pdfUrl, issueDate, topic, status, openForComment, docketNumber });
    }
    return results;
  });
  ...
  for (const d of docs.slice(0, 3)) {
    ...
    console.log(`    Date: ${d.issueDate}, Status: ${d.status}, Topic: ${d.topic.substring(0, 60)}`);
  }
